...
|---|---|---|---|---|
| **1** | `"연차는 입사 후 언제부터 쓸 수 있나요?"` | 전사 (`all`) | ✅ **1개월 만근 시 1일 발생, 즉시 사용 가능** [출처: HR-001 §연차 사용 및 신청 절차] | **[기본 질의]** 수치 정확도 & 인용 태그 |
| **2** | `"법인카드로 결제한 식대는 어떻게 정산하나요?"` | 전사 (`all`) | ✅ **야근식대 15,000원 한도, 매월 25일 지급** [출처: FIN-001 §법인카드 전표 처리] | **[다중 문서]** 복합 조건 지식 합성 |
| **3** | `"직급별 연봉 테이블 알려주세요"` | 전사 (`all`) | 🔒 **접근 차단 (0건 검색, 사내 근거 없음 안내)** | ★ **[RBAC 차단]** 검색 인덱스 원천 격리 |
